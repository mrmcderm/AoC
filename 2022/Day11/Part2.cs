    ...
    internal class Part2 : IPuzzle
    {
        ...
        public void Solve()
        {
            ...
            foreach (var rule in monkeyRules)
            {
                ...
                monkey.Items = decomposedRule[1].Split(": ")[1].Split(", ").Select(_ => Int128.Parse(_)).ToList();
                ...
                //Parse Test Value
                monkey.TestValue = Int128.Parse(decomposedRule[3].Split("Test: divisible by ")[1]);
            ...
            for (int round = 1; round <= 10000; round++)
            {
                foreach (var monkey in monkeys)
                {
                    ...
                    var items = new List<Int128>(monkey.Items);
                    ...
                    foreach (var item in items)
                    {
                        ...
                        var worryLevel = GetWorryLevel(monkey.Operand1 == "old" ? old : Int128.Parse(monkey.Operand1), monkey.Operator, monkey.Operand2 == "old" ? old : Int128.Parse(monkey.Operand2));

                        monkey.InspectionCount++;

                        if (worryLevel % monkey.TestValue == 0)
                        {
                            monkeys.First(_ => _.Id == monkey.TargetMonkeyIdWhenTestTrue).Items.Add(old);
                        }
                        else
                        {
                            monkeys.First(_ => _.Id == monkey.TargetMonkeyIdWhenTestFalse).Items.Add(old);
                        }
                    }
                }
            }
        ...
        private static Int128 GetWorryLevel(Int128 operand1, string @operator, Int128 operand2)
        {
